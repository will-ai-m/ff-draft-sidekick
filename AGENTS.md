...
Half-PPR by default; `npm run tiers:positional -- --format ppr` pulls the full-PPR RB/WR/TE
pages instead (the app drafts on either format since 2026-09-02 — the Rankings format toggle).

Writes `research/fantasypros-positional-tiers-<date>.md` (half-PPR) or
`research/fantasypros-positional-tiers-ppr-<date>.md` (full PPR) — date from the newest page's
own `last_updated` stamp; same-day re-runs overwrite with the fresher pull — and prints a digest:
...
- **The engine uses these same tiers**: the app fetches the QB/RB/WR/TE tier pages of the attach's
  rankings format itself at every attach for FR-10's tier urgency (K/DST are report-only — they
  never enter engine math).
- **Snapshots freeze at attach** (AC-29): this command never updates a running draft. To draft
  on a newer board: Detach → re-attach, then pick the format on the attach screen; the pre-draft check shows what it got and warns

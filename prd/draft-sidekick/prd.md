...
**Path:** launch the local app → paste the draft's URL/ID (or pick from the optional username-based convenience list) → app attaches and shows the draft's teams and owners → user confirms it's the right draft (and clicks their slot if auto-detect can't) → app runs the pre-draft check: ECR/ADP snapshot ages (warn if stale), player-matching results with any unmatched entries listed, league settings summary (teams, roster slots, scoring) → full board ingest.
...
- [ ] When a stored Sleeper username exists, the system shall additionally offer that user's discoverable drafts for the season as a convenience list (mocks may not appear in discovery — unverified; paste always works).
...
- [ ] For each team in the window, the system shall display its most likely position(s) to draft, with likelihood derived from its need-vector weights normalized to sum to 1 (before any FR-7 tendency adjustment), and example likely players drawn from ADP order within those positions.
...
**Acceptance criteria**
- [ ] At all times, the candidate list shall display the top available candidates (default 8 — 🔶 AS-5) in ECR order, plus any additional row required by FR-10's highlight; each row shows overall ECR rank, positional rank, ADP (so the ECR-vs-market gap is visible per row — 🔶 AS-8), and — while the user has a subsequent pick — survival percentage with band (every displayed skill-position row has one; the FR-8 universe extends to cover it).
...
- [ ] Each plan shall be scored as the sum of two overall-ECR-rank terms: the rank of the best available player at the plan's now-position, and the expected rank of the best available player at the plan's next-position at the user's next turn — that expectation computed from the same FR-8 Monte Carlo runs (per run, the best surviving rank at that position; averaged across runs), never from marginal survival probabilities combined as if independent. Runs in which no player at that position survives score at the snapshot's last-ranked overall ECR rank + 1. The lower total wins (🔶 AS-2).
...
- [ ] The system shall display the winning plan and the closest alternative, with the survival fact that separates them (e.g., "RB shelf: 2 of 5 likely gone by your next pick").
- [ ] When plan totals are within 3 overall-ECR ranks of each other (🔶 AS-5), the system shall state that and fall back to the higher-ECR current pick rather than manufacturing a preference; the too-close statement merges with FR-9's within-noise line rather than rendering twice.
...
| OQ-3 | *(Added 2026-08-27.)* Behavioral ADP source alignment: FR-8 samples opponent picks from FFC ADP, but both 08-27 rehearsal rooms drafted by Sleeper's own board — `trace:calibrate` measured the cost ("likely available" p ≥ .75 survived 50–80% vs ~0.90 predicted; e.g. Bowers FFC-ADP 44 drafted #23). **Feasibility verified 2026-08-27:** the FantasyPros half-PPR ADP page (`/nfl/adp/half-point-ppr-overall.php`) embeds per-source, per-player values including a Sleeper column (`src_4350`), keyed by the same FP player ids FR-4's ECR matching already joins on — so a Sleeper-aligned ADP snapshot is the same page-embed technique as OQ-2. Open: implement as an ADP source (FFC fallback; pool semantics differ — FP is per-format, not per-team-count), and re-measure calibration on rehearsal #3 before/after | FR-8's survival trustworthiness in real Sleeper rooms | willyu | Before first real draft |

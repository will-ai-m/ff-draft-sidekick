...
**Acceptance criteria**
- [ ] At all times, the candidate list shall display the top available candidates (default 8 — 🔶 AS-5) in ECR order, plus any additional row required by FR-10's highlight; each row shows overall ECR rank, positional rank, FantasyPros tier (surfaced 2026-08-31 — the dropoff structure FR-10's urgency facts cite), ADP (so the ECR-vs-market gap is visible per row — 🔶 AS-8), and — while the user has a subsequent pick — survival percentage with band (every displayed skill-position row has one; the FR-8 universe extends to cover it).
...
- [ ] When plan totals are within 0.75 projected pts/gm of each other (🔶 AS-5; `planTotalTooClosePoints`, replacing the rank-scale `planTotalTooCloseEcrRanks` 2026-08-31), the system shall state that and fall back to the better-consensus current pick — of the two plans' now-picks, the player with the better overall ECR — rather than manufacturing a preference; the too-close statement merges with FR-9's within-noise line rather than rendering twice. *(The 2026-08-31 wording is deliberate: the prior "higher-ECR" phrasing shipped as a rank-delta comparison that let two same-position plans occupy both slots and sent the worse-consensus player out at pick 1.)*
